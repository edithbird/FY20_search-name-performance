...
SR_Nov_trythisGroupedStatus <- SR_Nov_trythisGrouped %>% group_by(Vendor, Class, Geo,
                                                    TargetX_SRMb__Status__c) %>% summarise(n = n())

SR_Nov_trythisGroupedStatus <- SR_Nov_trythisGroupedStatus %>% spread(TargetX_SRMb__Status__c, n) %>% mutate_at(4:5, ~replace(., is.na(.), 0))
...
JR_Nov_trythisGroupedStatus <- JR_Nov_trythisGrouped %>% group_by(Vendor, Class, Geo,
                                                    TargetX_SRMb__Status__c) %>% summarise(n = n())
